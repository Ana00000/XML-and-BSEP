...
public class CertificateController {
	...
	public ResponseEntity<CertificateDTO> createCertificate(@RequestBody IntermediateCA intermediateCA) {
		try {
			...
			X509Certificate cert = new CertificateGenerator().generateCertificate(subject, issuer);
	...
	private void writingCertificateInFile(KeyPair keyPairIssuer, IntermediateCA intermediateCA, KeyStore keyStore,
	...
	private Subject generateSubject(IntermediateCA intermediateCA) {
		...
		return new Subject(keyPairSubject.getPublic(), builder.build(), intermediateCA.getSerialNumber(), startDate,
				endDate);
